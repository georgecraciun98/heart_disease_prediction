...
class MonitoredDataDoctor(generics.ListAPIView):
    model= Patient
    ...
    def get_queryset(self,id):
        ...
        queryset = MonitoredData.objects.filter(patient_id=patient_id)
        return queryset
    def get_by_activity(self,data_type,start_time,end_time,id):
        patient_id=id
        queryset = MonitoredData.objects.filter(patient_id=patient_id,data_type=data_type,
                                                start_time__gte=start_time,end_time__lte=end_time)
        return queryset
    ...
    def post(self, request,pk):
        ...
        try:
            ...
            try:
                ...
                end_time_obj = datetime.now()
            end_time_tz=end_time_obj.replace(tzinfo=pytz.UTC)
            queryset = self.filter_queryset(self.get_by_activity(data_type, start_time_tz, end_time_tz,patient.id))
